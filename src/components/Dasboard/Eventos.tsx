...
function Eventos() {
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post('https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/uploadeventos', formData, {
        headers: {
 ...
 */
  const handleDelete = async (id: string) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar esta imagen?')) {
      try {
        await axios.delete(`https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/eventos/${id}`);
        const response = await axios.get('https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/eventos');
        setImages(response.data);
      } catch (error) {
