...
function Galeria() {
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post('https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/upload', formData, {
        headers: {
 ...
 */
  const handleDelete = async (id: string) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar esta imagen?')) {
      try {
        await axios.delete(`https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/gallery/${id}`);
        const response = await axios.get('https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/gallery');
        setImages(response.data);
      } catch (error) {
