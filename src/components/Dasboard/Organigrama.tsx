...
function Organigrama() {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await axios.get('https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/organigrama/');
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post('https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/uploadorganigrama', formData, {
        headers: {
...
/*   const handlePreview = (image: Image) => {
    ...
    const previewModal = new window.bootstrap.Modal(document.getElementById('previewModal') as Element);
    previewModal.show();
  }; */
 ...
 */
  const handleDelete = async (id: string) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar esta imagen?')) {
      try {
        await axios.delete(`https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/organigrama/${id}`);
        const response = await axios.get('https://heroic-torrone-6e5f03.netlify.app/.netlify/functions/api/api/organigrama');
        setImages(response.data);
      } catch (error) {
